...
class GeochemDB:
    ...
    def matchrows(self, table, values, columns):
        ...
        table_arr = table_df[columns].to_numpy()
    ...
    def matchcolumns(self, table, df_cols, score_threshold=96.0):
        ...
        cols_sq_df = pd.DataFrame(
            columns=["id", "name", "type", "c1", "c2", "c3"], data=columns_info
        )
        ...
        # do matching
        for col in df_cols:
            sq_col_match, score = process.extractOne(col, sq_cols)
            # must meet threshold
            if score >= score_threshold:
    ...
    def matchsamples_df(self, df, score_threshold=96.0):
        ...
        idx, sample_match_dict = self.matchrows_strings(
            "Samples", samples_unique, "name", score_threshold=score_threshold
        )
        ...
        samples_not_matched = set(samples_unique) ^ set(sample_names_matched)

        if len(samples_not_matched) > 0:
            print(f"Sample names not matched:\n{samples_not_matched}")

        # keep only rows with matched samples (indexing into df)
        idx_samples = np.array([x in sample_names_matched for x in df["sample"]])
    ...
    def insert_rows(self, table, columns, values):
        ...
        """
        assert len(columns) == len(values[0]), "Must have value for each column."
    ...
    def update_rows(
        self, table, match_columns, match_values, update_columns, update_values
    ):
        """
        ...
        # assemble values
        values = [
            update_value + match_value
            for update_value, match_value in zip(update_values, match_values)
        ]
    ...
    def measurements_update(self, df_measurements):
        ...
        # update in database
        self.update_rows(
            "Measurements", cols_match, match_values, cols_update, update_values
        )

        print("Updated:\n" + f"{len(df_measurements)} measurements")

    def measurements_add(
        self, df_measurements, df_analyses, df_aliquots, score_threshold=98
    ):
        """
        ...
        df_analyses : pandas.DataFrame
            ...
            analysis, aliquot, date, instrument, technique
        ...
        # check that required columns are present in dataframes, if not alert which ones are missing
        missing_cols = set(cols_meas) - set(df_measurements.columns)
        assert not missing_cols, f"Missing columns in df_measurements: {missing_cols}"
        cols_analyses = ["analysis", "aliquot", "date", "instrument", "technique"]
        missing_cols = set(cols_analyses) - set(df_analyses.columns)
        assert not missing_cols, f"Missing columns in df_analyses: {missing_cols}"
        cols_aliquots = ["aliquot", "sample", "material"]
        missing_cols = set(cols_aliquots) - set(df_aliquots.columns)
        assert not missing_cols, f"Missing columns in df_aliquots: {missing_cols}"

        # make sure that all analyses in df_measurements are also in
        # df_analyses
        assert set(df_analyses["analysis"].unique().tolist()) == set(
            df_measurements["analysis"].unique().tolist()
        ...
        assert set(df_analyses["aliquot"].unique().tolist()) == set(
            df_aliquots["aliquot"].unique().tolist()
        ...
        if np.sum(idx) == len(df_measurements):
            print(
                "All measurements already in database, use "
        ...
        idx_aliquots = ~self.matchrows(
            "Aliquots", df_aliquots["aliquot"].values, "aliquot"
        )
        if np.any(idx_aliquots):
            cur_values = df_aliquots.loc[idx_aliquots][cols_aliquots].values
            self.insert_rows("Aliquots", cols_aliquots, cur_values.tolist())
            print(f"Added: {np.sum(idx_aliquots)} aliquots.")

        # create necessary analyses
        idx_analyses = ~self.matchrows(
            "Analyses", df_analyses["analysis"].values, "analysis"
        )
        ...
        if np.any(idx_analyses):
            ...
            self.insert_rows("Analyses", cols_analyses, cur_values.tolist())
        ...
        QuantitiesMeasurementUnits_df = pd.read_sql_query(
            "SELECT * from QuantitiesMeasurementUnits", self.con
        )
        for ii, row in df_measurements[["quantity", "measurement_unit"]].iterrows():
            idx = (QuantitiesMeasurementUnits_df["quantity"] == row["quantity"]) & (
                QuantitiesMeasurementUnits_df["measurement_unit"]
                == row["measurement_unit"]
            )
            if not np.any(idx):
                raise ValueError(
                    f"Quantity and measurement_unit pair not in QuantitiesMeasurementUnits table: {row['quantity']}, {row['measurement_unit']}"
                )

        # then add measurements gracefully (close connection if error)
        self.insert_rows(
            "Measurements", cols_meas, df_measurements[cols_meas].values.tolist()
        )
    ...
    def measurements_by_sample(self, samples):
        ...
        df_analyses = df_analyses.merge(
            df_aliquots, how="left", left_on="aliquot", right_on="aliquot"
        )
        df_measurements = df_measurements.merge(
            df_analyses, how="left", left_on="analysis", right_on="analysis"
        )
    ...
    def measurements_by_aliquot(self, aliquots):
        ...
        if len(aliquots) == 1:
            sql = (
                f'SELECT aliquot, sample FROM Aliquots WHERE aliquot = "{aliquots[0]}"'
            )
        else:
...
def measurements2aliquot(df_measurements):
    """pivot table from measurements to aliquots
    Given a dataframe of measurements as generated by :py:meth:`GeochemDB.measurements_by_sample()` or :py:meth:`GeochemDB.measurements_by_aliquots()`, create pivot table with aliquots on index.
    ...
    -------
    pd.DataFrame
        DataFrame with geochemical measurements indexed by aliquot and sample.
    """
    df_aliquots = df_measurements.pivot_table(
        columns=["quantity"],
...
def uncertainty_spec(df_measurements, uncertainty_unit="1 sigma (absolute)"):
    """ "Convert to consistent uncertainty unit.

    Parameters
    ----------
    df_measurements : pd.DataFrame
        Dataframe of measurements output by :py:meth:`GeochemDB.measurements_by_sample()` or :py:meth:`GeochemDB.measurements_by_aliquots()`.
    uncertainty_unit : str, optional
        Desired uncertainty unit. Options are "1 sigma (absolute)", "2 sigma (absolute)", "1 sigma (relative)", and "2 sigma (relative)". The default is "1 sigma (absolute)".

    Returns
    -------
    pd.DataFrame
        DataFrame with geochemical measurements converted to the specified uncertainty unit.
    """

    def convert_to_1sigma(row):
        if row["uncertainty_unit"] == "1 sigma (absolute)":
            return row["uncertainty"]
        elif row["uncertainty_unit"] == "2 sigma (absolute)":
            return row["uncertainty"] / 2
        elif row["uncertainty_unit"] == "1 sigma (relative)":
            return row["mean"] * row["uncertainty"]
        elif row["uncertainty_unit"] == "2 sigma (relative)":
            return row["mean"] * row["uncertainty"] / 2

    def convert_to_2sigma(row):
        if row["uncertainty_unit"] == "1 sigma (absolute)":
            return row["uncertainty"] * 2
        elif row["uncertainty_unit"] == "2 sigma (absolute)":
            return row["uncertainty"]
    ...
    def convert_to_1sigma_relative(row):
        if row["uncertainty_unit"] == "1 sigma (absolute)":
            return row["uncertainty"] / row["mean"]
        elif row["uncertainty_unit"] == "2 sigma (absolute)":
            return row["uncertainty"] / 2 / row["mean"]
        elif row["uncertainty_unit"] == "1 sigma (relative)":
            return row["uncertainty"]
        elif row["uncertainty_unit"] == "2 sigma (relative)":
            return row["uncertainty"] / 2

    def convert_to_2sigma_relative(row):
        if row["uncertainty_unit"] == "1 sigma (absolute)":
            return row["uncertainty"] * 2 / row["mean"]
        elif row["uncertainty_unit"] == "2 sigma (absolute)":
            return row["uncertainty"] / row["mean"]
    ...
    if uncertainty_unit == "1 sigma (absolute)":
        df_measurements["uncertainty"] = df_measurements.apply(
            lambda row: convert_to_1sigma(row),
            axis=1,
        )
        df_measurements["uncertainty_unit"] = "1 sigma (absolute)"
    elif uncertainty_unit == "2 sigma (absolute)":
        df_measurements["uncertainty"] = df_measurements.apply(
            lambda row: convert_to_2sigma(row),
            axis=1,
        )
        df_measurements["uncertainty_unit"] = "2 sigma (absolute)"
    elif uncertainty_unit == "1 sigma (relative)":
        df_measurements["uncertainty"] = df_measurements.apply(
            lambda row: convert_to_1sigma_relative(row),
            axis=1,
        )
        df_measurements["uncertainty_unit"] = "1 sigma (relative)"
    elif uncertainty_unit == "2 sigma (relative)":
        df_measurements["uncertainty"] = df_measurements.apply(
            lambda row: convert_to_2sigma_relative(row),
            axis=1,
        )
        df_measurements["uncertainty_unit"] = "2 sigma (relative)"

    return df_measurements


def aliquot_average(df_measurements, method="weighted mean"):
    """
    ...
    df_measurements : pd.DataFrame
        Dataframe of measurements output by :py:meth:`GeochemDB.measurements_by_sample()`.
    method : str, optional
        Method for averaging duplicate measurements. Currently only supports "weighted mean". The default is "weighted mean".
    ...
    df_measurements = uncertainty_spec(
        df_measurements, uncertainty_unit="1 sigma (absolute)"
    )

    def weighted_mean(means, uncertainties):
        weights = 1 / uncertainties**2
        mean = np.sum(means * weights) / np.sum(weights)
        variance = (np.sum(means**2 * weights) / np.sum(weights) - mean**2) * (
            len(means) / (len(means) - 1)
        )
        return mean, np.sqrt(variance)

    def _agg(group):
        all_quantities = group.columns.get_level_values(-1).unique()

        has_uncertainty = set()
        # identify columns with uncertainty values
        for col in group.columns.unique():
            el, level1 = col[0], col[1]
            if level1 == "uncertainty":
                has_uncertainty.add(el)

        result = {}
        for el in all_quantities:
            means = group[(el, "mean")].values

            if el in has_uncertainty:
                # Both mean and uncertainty available → weighted mean
                uncerts = group[("uncertainty", el)].values
                if method == "weighted mean":
                    w_mean, w_unc = weighted_mean(means, uncerts)
                result[(el, "mean")] = w_mean
                result[(el, "uncertainty")] = w_unc
            else:
                # Only mean available → arithmetic mean
                result[(el, "mean")] = np.mean(means)

        s = pd.Series(result)
        s.index = pd.MultiIndex.from_tuples(
            [(k[0], k[1]) for k in s.index], names=group.columns.names
        )
        return s

    df_aliquots = measurements2aliquot(df_measurements)

    # isolate duplicated measurements
    df_aliquots_dups = df_aliquots[df_aliquots.index.duplicated(keep=False)]
    df_aliquots_dups_avg = df_aliquots_dups.groupby(level="aliquot").apply(_agg)
